...
;lookup-fundef: <id> <list>[fundefs]-> list fundef

(define (lookup-fundef id fundefs)
(match fundefs
  [(list) ( error "my friend, that does not exist " id)]
[(cons function tail ) (let ([f-id (fundef-fname function)])
        (if (eq? f-id id) function (lookup-fundef id tail))
 )]
 )
)
