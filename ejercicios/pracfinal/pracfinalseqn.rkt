...
[(add l r)
     ; Evaluar r con el sto actual
     (def (v*s r-val r-sto) (interp r env sto))
     ; Evaluar l con el sto modificado por r
...
;Modifica el intérpreta para que evalúe la suma de derecha a izquierda y escribe pruebas que demuestren que el resultado cambia de acuerdo al orden de evaluación. (2pts)


(test (run '{+ 2 {+ 3 4}}) 9)  ; el resultado es igual en ambos tipos de evaluacion


; si introducimos side effects, hay casos donde la evaluacin importa 

(test
  (run
    '{with {result {newbox 0}}
      {seqn
        {setbox result {+ 2 3}} ; primero set
        {openbox result}}         ; luego open
    }
  )
  5)
...
(test
  (run
    '{with {result {newbox 0}}
      {seqn
        {openbox result}          ; primero open
        {setbox result {+ 2 3}}   ; luego set 
      }
    }
  )
  0) ; en este caso el orden de evaluacion importa





;Permite que seqn ya no acepte sólamente un par de instrucciones, sino que procese n instrucciones, escribe pruebas para esa ejecución. (4pts)
...
;Por ahora, cada cambio en sto implica agregar un nuevo registro encima de todos los anteriores. Modifica la implementación para que, sin usar mutación, se elimine el último registro del sto antes de actualizarlo. (4pts)
