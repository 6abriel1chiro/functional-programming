...
(define (interp expr env sto)
  (match expr
    ...
    [(set id e)
     ; 0. buscar en el ambiente el id-loc
     (def id-loc (env-lookup id env)) 
     ; 1. interp e
     (def (v*s e-val e-sto) (interp e env sto))
     ; 2. extend sto con el loc de id
     (v*s (voidV) (extend-sto id-loc e-val e-sto))
     ]
))
...
(test (run '{with {v {newbox 0}}
            {with {f {fun {x} {set x {newbox 5}}}}
                  {seqn {f v}
                        {openbox v}}}}) 0)
; caja / objeto, objeto es como una caja glorificada
(test (run '{with {v {newbox 0}}
            {with {f {fun {x} {setbox x 5}}}
                  {seqn {f v}
                        {openbox v}}}}) 5)