...
(deftype Expr
  ...
  [rec id-name named-expr body-expr ]      ; (rec)
  [id name]                               ; <id> 
...
(define (parse src)
  (match src
    ...
    ;azucar sintactico
     [(list 'fun (list args ...) body) (fun args body)]
...
(define (interp expr env)
  (match expr
    ...
    [(fun arg body) (closureV arg body env)] ; Por ahora, devolvemos la misma expresion que nos llego
   [(app f e)
     (match (strict (interp f env args))
       [(closureV fn-args fn-body fn-env)
        (if (= (length fn-args) (length e))
            (interp fn-body (foldr (lambda (a b) (extend-env a b fn-env)) (zip fn-args (map (lambda (arg) (strict (interp arg env args))) e)) empty-env))
...
(run '(fun (x y) (+ x y))) ; Creates a function taking two arguments and returning their sum
