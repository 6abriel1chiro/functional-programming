...
; apuntes mon 20 nov : theading pattern (AST)
; gpl vs dsl
;  dsl deben ser mejores para la tarea para lo que estan hechos y tener alguna caracteristica general
;
...
(define pet
  (OBJECT
   ...
   (
    [method speak ()
      (cond ((= breed "dog") (display "Woof!")
            ((= breed "cat") (display "Meow!"))
            ))]
    [method eat (food) (display (list name) "is eating" (list food))]
    )
   )
  )
